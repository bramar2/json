...
namespace json {
	...
	// JsonElement* path(JsonElement* root, std::string_view query) {
	// 	if (query.empty()) return nullptr;

	// 	size_t ptr = 1, sz = query.size(); 
	// 	if (sz == 1u && query[0] == '$') {
	// 		return root;
	// 	}
	...
	// 			root = obj->at(key);
	// 		} else if (query[ptr] == '[' && std::isdigit(query[ptr + 1])) {
	// 			if (root->type() != JsonType::Array) {
	// 				return nullptr;
	// 			}
	// 			JsonArray* arr = static_cast<JsonArray*>(root);
	...
	// 					}
	// 				}
	// 				if (key.size() > 8u || ptr >= sz || query[ptr] != ']') {
	// 					return nullptr;
	// 				}
	// 				ptr += 1;

	// 				size_t idx = static_cast<size_t>(stoull(key));
	// 				if (idx >= arr->elements.size()) {
	// 					return nullptr; 
	// 				}
